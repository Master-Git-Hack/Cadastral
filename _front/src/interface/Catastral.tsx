...
export default function Catastral() {
  ...
  const [files, setFiles] = useState<Array<string>>([
    `report_${new Date().toISOString()}_temp.pdf`,
  ]);
  ...
  const handleFiles = async () => {
    if (
      window.confirm(
        "¿Está seguro de generar el reporte?, una vez generado la página será actualizada y se perderán los cambios."
      )
    ) {
      await axios({
        method: "POST",
        ...
        .then((response: any) =>
          FileSaver.saveAs(
            response.data,
            `report_${new Date().toISOString()}.pdf`
          )
        )
          ...
          {renderPages.map((key_id: number) => (
            <tr className="row p-1">
              <td className="col-12 text-start">
                <Avaluos key={key_id} key_id={key_id} files={files} />
              </td>
            </tr>
          ))}
        </tbody>
