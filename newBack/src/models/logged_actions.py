...
class _LoggedActions(Base):
    ...
    __table_args__ = CheckConstraint(
        "action = ANY (ARRAY['I'::text, 'D'::text, 'U'::text])"
    )
...
class _Read:
    ...
    async def all(
    ...
    ) -> _LoggedActions or _Schema or None:
        """
        ...
        """
        try:
            response = db.session.query(_LoggedActions).all()
            if to_list:
    ...
    async def by_id(
        ...
        """
        try:
            response = db.session.query(_LoggedActions).get(_id)
            if to_dict:
...
class LoggedActions:
    ...
    ) -> Tuple[bool, _LoggedActions or _Schema or None]:
        """
...
and return a tuple with the results
        ...
        """
        current_entity = db.session.query(_LoggedActions).get(_id)
    ...
    @staticmethod
    def to_dict(entity: _LoggedActions, exclude: Optional[list] = None) -> _Schema:
    ...
    @staticmethod
    def to_list(entities: _LoggedActions, exclude: Optional[list] = None) -> list:
        """Pass a model instance to a dict
        Args:
