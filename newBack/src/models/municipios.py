"""
Handle everything related to the Municipios model
Define the Municipios model.
Define the Municipios schema.
Define the Municipios crud.
...
class _Municipios(Base):
    """Model for Municipios.
    Example:
        >>> Municipios(data:dict)
    Args:
...
class _Read:
    ...
    async def all(
    ...
    ) -> _Municipios or _Schema or None:
        """
        ...
        """
        try:
            response = db.session.query(_Municipios).all()
            if to_list:
    ...
    async def by_id(
        ...
        """
        try:
            response = db.session.query(_Municipios).get(_id)
            if to_dict:
...
class Municipios:
    ...
    ) -> Tuple[bool, _Municipios or _Schema or None]:
        """
...
and return a tuple with the results
        ...
        """
        current_entity = db.session.query(_Municipios).get(_id)
    ...
    @staticmethod
    def to_dict(entity: _Municipios, exclude: Optional[list] = None) -> _Schema:
    ...
    @staticmethod
    def to_list(entities: _Municipios, exclude: Optional[list] = None) -> list:
        """Pass a model instance to a dict
        Args:
