...
const {REACT_APP_API_URL} = process.env;
  const {averageUnitValue,id,type} =selectHomologation(getState());
  const payload = {
    type,
    value:averageUnitValue
  }
  const url = `${REACT_APP_API_URL}/homologation/${id}`;
  const response = await axios.patch(url, payload,config("json"))
...
export const sendPatchRequest = createAsyncThunk(
  ...
  async (state: any) => {
    const { id, type, value } = state;
    const payload = {
      type,
      value,
    };
    const url = `/homologation/${id}`;
