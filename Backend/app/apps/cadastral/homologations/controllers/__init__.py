...
class Controller:
        ...
        if len(responseHomologation) > 0:
            ...
            data = {
                ...
                "areas": {
                    "subject": {
                        ...
                        if responseJustipreciacion[0]["superficie"] is not None
                        else 1
                    }
                },
            }
            if self.type == "TERRENO":
                data["reFactor"] = {
                    ...
                    if responseJustipreciacion[0]["factor"] is not None
                    else 1
                }
            else:
                data["ages"] = {
                    "subject": {
                        ...
                        if responseJustipreciacion[0]["edad"] is not None
                        else 1
                    }
                }
            return data
    ...
    def patch(
        ...
        homologation = Homologation(self.id, self.type)
        if exists:
            response = homologation.patchHomologation(
                record,
                factors,
                results,
                averageUnitCost,
            ...
            response = homologation.insertHomologation(
                factors, results, averageUnitCost, registration, appraisalPurpose
            )
        if response != -1:
