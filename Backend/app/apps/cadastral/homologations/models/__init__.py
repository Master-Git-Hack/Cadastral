...
class Homologation:
    ...
    def getJustipreciacion(self):
        if self.type == "RENTA":
            query = Template(open(f"{HOMOLOGATIONS_PATH}{MODELS_PATH}/getJustipreciacionRenta.sql").read()).render(
                ID=self.id
            )
        else:
            query = Template(open(f"{HOMOLOGATIONS_PATH}{MODELS_PATH}/getJustipreciacionTerreno.sql").read()).render(
                ID=self.id
            )
        return self.query(query)

    def getHomologation(self, registration):
        query = Template(open(f"{HOMOLOGATIONS_PATH}{MODELS_PATH}/getHomologation.sql").read()).render(
            REGISTRATION=registration, TYPE=self.type.lower()
        )
        return self.query(query)
