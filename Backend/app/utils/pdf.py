...
class PDF():
    def __init__(self, zoom=1, page_size="A4", margins = {
            "top":15,
            "bottom":None,
            "left":None,
            "right":None
        ...
        for document in self.collection:
            ...
            with open(file, 'w') as file:
                file.write(_template.render(
                    ...
                    CNA_TIPO = item_validator(document[0]["cna_tipo"]).split('|')[0] if item_validator(document[0]["cna_tipo"]).find('|') != -1 else item_validator(document[0]["cna_tipo"]),
                    ...
                    CNB_TIPO = item_validator(document[0]["cnb_tipo"]).split('|')[0] if item_validator(document[0]["cnb_tipo"]).find('|') != -1 else item_validator(document[0]["cnb_tipo"]),
                    ...
                    CNC_TIPO = item_validator(document[0]["cnc_tipo"]).split('|')[0] if item_validator(document[0]["cnc_tipo"]).find('|') != -1 else item_validator(document[0]["cnc_tipo"]),
                    ...
                    CND_TIPO = item_validator(document[0]["cnd_tipo"]).split('|')[0] if item_validator(document[0]["cnd_tipo"]).find('|') != -1 else item_validator(document[0]["cnd_tipo"]),
                    ...
                    INCREMENTO_ESQUINA_SUPERFICIE = withDecimals(item_validator(document[0]["incr_esq_superficie"]), 2),
                    ...
                    INCREMENTO_ESQUINA_VALOR_PARCIAL = asCurrency(item_validator(document[0]["incr_esq_valor_parcial"])),
                    ...
                    SUPERFICIE_TOTAL_CONSTRUCCION = withDecimals(item_validator(document[0]["sup_total_construccion"]), 2),
