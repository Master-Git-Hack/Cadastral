...
import type { TableComponent, TableProps } from "./table.types";
...
export const Header = (props: TableProps) => (
	<thead className={className(props)}>{props.children}</thead>
);
export const Body = (props: TableProps) => (
	<tbody className={className(props)}>{props.children}</tbody>
);
export const Footer = (props: TableProps) => (
				...
				<tr>
					{header &&
						header.map((item: string, index: number) => (
							<th key={`table header component for ${name} ${index}`}>
								{item}
							</th>
						))}
				</tr>
