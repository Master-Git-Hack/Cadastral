...
export const reducers = {
    ...
    updateStateByKeys: (state: APIState, { payload: { key, value } }: PayloadAction<updateStateByKeys>) => {
        if(key!==undefined && value!==undefined)
        state[key] = value;
    },
    updateStateBySubKeys: (state: APIState, { payload: { key, parentKey, value } }: PayloadAction<updateStateBySubKeys>) => {
