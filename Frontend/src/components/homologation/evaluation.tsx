...
import { selectHomologation, setResults } from "../../features/homologations/homologationsSlice";
import { Table } from "./evaluation/table";
export default function Evaluation(props:any) {
  const dispatch = useAppDispatch();
  useEffect(() => {dispatch(setResults())},[]);
  ...
  const headerForFactors=[
    ...
    'FUbic.',
    'FZo.',
    new URLSearchParams(window.location.search).get("tipo")?.toUpperCase() === "TERRENO" && 'FEd.',
