...
class Cadastral:
    ...
    def define_collection(self):
        query = Template(
            open(f"{APPRAISAL_PATH}{MODELS_PATH}/Collection2Ids.sql").read()
        ).render(
            COLLECTION=self.collection, YEAR=self.year, START=self.start, END=self.end
        )
