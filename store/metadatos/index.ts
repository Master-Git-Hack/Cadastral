		...
		(set, get) => ({
			...
			getMetadato: async (uid: string, isTemporal: boolean = false) => {
				const {
					...
					`metadatos${isTemporal ? "/temporal/" : "/"}${uid}`,
					{},
				);
				set(data);
			},
			postMetadato: async () => {
				...
				return await api.post("metadatos/create", {
					uid,
					db_name,
					table_name,
					schema_name,
			...
			patchMetadato: async () => {
				...
				return await api.patch(`metadatos/${uid}`, {
					uid,
					db_name,
					table_name,
					schema_name,
			...
			postTemporal: async () => {
				...
				return await api.post("metadatos/temporal/create", {
					datos: {
						db_name,
						table_name,
						schema_name,
