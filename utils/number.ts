...
export const asFancyNumber = (
	value: number,
	{ decimals = 2, style = "decimal" }: IFancyNumber,
	fixed = true,
): string => {
	const currency = style === "currency" ? "MXN" : undefined;
	const unit = style === "percent" ? 100 : 1;
	decimals = style === "percent" ? 0 : decimals;
	...
	return fixed ? format.format(Number(current) / unit) : current;
};
/**
...
export const roundNumber = (value: number, round = -1): number => {
	if (round < 0) return Number(value.toFixed(2));
