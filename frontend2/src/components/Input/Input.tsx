...
export const Switch = (props: SwitchProps): JSX.Element => {
	const { size, checkedText, uncheckedText, checked, onChange } = props;
	...
	return (
		<Toggle
			checked={checked}
			size={size ?? "md"}
			checkedChildren={checkedText ?? "Open"}
			unCheckedChildren={uncheckedText ?? "Close"}
			onChange={(result: boolean, event: any) => {
				setLoading(true);
...
export const File = ({
	...
	return (
		<div className={`input-group input-group-sm ${className}`}>
			...
			{value && (
				<>
					<Button type={"success"} onClick={() => FileSaver.saveAs(value, filename)}>
						Descargar
					</Button>
					<Button type={"danger"} outline onClick={handleRemove}>
						Eliminar
					</Button>
				</>
			)}
