...
export const reducers = ({
    ...
    add: (state: JustipreciacionState, { payload: { key, value } }: PayloadAction<PayloadByKey>) => {
        if(value!==undefined){
            state[key] = value;
        }
    },patch: (state: JustipreciacionState, { payload: { key, value } }: PayloadAction<PayloadByKey>) => {
        if( value!==undefined && key in state ){
            state[key] = value;
        }
    },
    loadTerreno:(state: JustipreciacionState, { payload:{sp1_superficie,sp1_factor}  }: PayloadAction<JustipreciacionState>) => {
